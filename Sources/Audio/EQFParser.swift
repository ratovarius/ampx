...
enum EQFParser {
    ...
    static func parse(_ data: Data) throws -> [EQPreset] {
        ...
        guard bytes.count >= self.header.count + 4 else { throw EQFError.truncated }

        let headerBytes = bytes.prefix(self.header.count)
        guard String(decoding: headerBytes, as: UTF8.self) == self.header else {
            throw EQFError.invalidHeader
        }

        var i = self.header.count + 4 // skip header + "\u{1A}!--" separator
        var presets: [EQPreset] = []

        while i + self.nameLength + self.bandCount + 1 <= bytes.count {
            let nameField = bytes[i ..< (i + self.nameLength)]
            ...
            for _ in 0 ..< self.bandCount {
                gains.append(self.eqfValueToDB(Int(bytes[i])))
                i += 1
            }
            let preamp = self.eqfValueToDB(Int(bytes[i]))
            i += 1
    ...
    static func write(_ presets: [EQPreset]) -> Data {
        ...
        for preset in presets {
            ...
            while name.utf8.count > self.nameLength - 1 {
                name.removeLast()
            }
            var nameField = [UInt8](name.utf8)
            nameField.append(contentsOf: Array(repeating: 0, count: self.nameLength - nameField.count))
            bytes.append(contentsOf: nameField)

            let gains = self.normalizedBands(preset.bandGainsDB)
            for gain in gains {
                bytes.append(self.dbToEQFValue(gain))
            }
            bytes.append(self.dbToEQFValue(preset.preampGainDB))
        }
        return Data(bytes)
    }
